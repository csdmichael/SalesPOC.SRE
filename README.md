...
Incident response plans and scheduled tasks are auto-generated by the SRE Agent platform once the knowledge graph finishes building from the managed resources in the resource group.

### Agent Extensions (Preview — Internal Tenants Only)

The `Microsoft.App/agents` API (`2025-05-01-preview`) exposes additional sub-resource endpoints that are currently restricted to **internal Microsoft tenants only**:

| Endpoint | Status | Description |
|---|---|---|
| `subagents` | Blocked | Register platform-level subagents |
| `skills` | Blocked | Register agent skills |
| `scheduledTasks` | Blocked | Configure platform-scheduled tasks |

**Error:** `Agent Extensions are not available for this tenant. This feature is restricted to internal tenants only.`

The `GET` (list) operations succeed on these endpoints (returning empty arrays), but `PUT` (create) operations are rejected. Until Microsoft makes Agent Extensions generally available, the 6 subagents defined in [src/subagents.py](src/subagents.py) run as **application-level code** inside the container — they are not registered as platform sub-resources. Scheduled tasks similarly run in-process via the async scheduler in [src/scheduler.py](src/scheduler.py).
