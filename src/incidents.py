...
INCIDENT_PLANS: list[IncidentPlan] = [
    ...
    IncidentPlan(
        ...
        runbook=[
            RunbookStep(1, "Identify queries", "Run sys.dm_exec_query_stats to find top CPU queries", automated=True,
                        command=f"az sql db show --name {settings.sql_database_name} --server {settings.sql_server_name} --resource-group {settings.azure_resource_group} --query '{{sku:currentSku,status:status}}'"),
            ...
            RunbookStep(3, "Scale up compute", "Temporarily scale to a higher vCore tier if load is legitimate", automated=True,
                        command=f"az sql db update --name {settings.sql_database_name} --server {settings.sql_server_name} --resource-group {settings.azure_resource_group} --service-objective S3"),
    ...
    IncidentPlan(
        ...
        runbook=[
            ...
            RunbookStep(2, "Check server status", "Verify SQL server is responding", automated=True,
                        command=f"az sql server show --name {settings.sql_server_name} --resource-group {settings.azure_resource_group} --query '{{state:state,fullyQualifiedDomainName:fullyQualifiedDomainName}}'"),
    ...
    IncidentPlan(
        ...
        runbook=[
            ...
            RunbookStep(3, "Scale storage", "Increase max database size", automated=True,
                        command=f"az sql db update --name {settings.sql_database_name} --server {settings.sql_server_name} --resource-group {settings.azure_resource_group} --max-size 250GB"),
        ],
    ),
    ...
    IncidentPlan(
        ...
        runbook=[
            RunbookStep(1, "Check RU consumption", "Review partition-level RU usage", automated=True,
                        command=f"az cosmosdb show --name {settings.cosmos_account_name} --resource-group {settings.azure_resource_group} --query '{{enableAutomaticFailover:enableAutomaticFailover,provisioningState:provisioningState}}'"),
            ...
            RunbookStep(3, "Scale RU throughput", "Increase provisioned or autoscale max RUs", automated=True,
                        command=f"az cosmosdb sql container throughput update --account-name {settings.cosmos_account_name} --database-name {settings.cosmos_database_name} --name {settings.cosmos_container_name} --resource-group {settings.azure_resource_group} --throughput 1000"),
    ...
    IncidentPlan(
        ...
        runbook=[
            ...
            RunbookStep(2, "Check storage account health", "Review storage account diagnostics", automated=True,
                        command=f"az storage account show --name {settings.storage_account_name} --resource-group {settings.azure_resource_group} --query '{{provisioningState:provisioningState,statusOfPrimary:statusOfPrimary}}'"),
    ...
    IncidentPlan(
        name="api_5xx_spike",
        resource_type="api",
        trigger_condition="Http5xx > 5 in 1min",
        severity=Severity.SEV1,
        description="API 5xx Spike",
        runbook=[
            RunbookStep(1, "Check GitHub Repository", "Check GitHub Repository for API and rollback to latest stable version if the error occurred after a deployment"),
            RunbookStep(2, "Check App Insights", "Review exception telemetry for root cause", automated=True,
                        command=f"az webapp log tail --name {settings.api_app_service_name} --resource-group {settings.azure_resource_group}"),
            RunbookStep(3, "Check dependencies", "Verify SQL DB, Cosmos DB, and Storage connectivity"),
            RunbookStep(4, "Restart API", "Restart the App Service", automated=True,
                        command=f"az webapp restart --name {settings.api_app_service_name} --resource-group {settings.azure_resource_group}"),
            RunbookStep(5, "Scale out", "Add instances if load is too high", automated=True,
                        command=f"az appservice plan update --name {settings.app_service_plan_name} --resource-group {settings.azure_resource_group} --number-of-workers 3"),
        ],
        auto_remediate=True,
    ),
    IncidentPlan(
    ...
    IncidentPlan(
        ...
        runbook=[
            ...
            RunbookStep(2, "Scale out", "Add more instances", automated=True,
                        command=f"az appservice plan update --name {settings.app_service_plan_name} --resource-group {settings.azure_resource_group} --number-of-workers 4"),
    ...
    IncidentPlan(
        ...
        runbook=[
            ...
            RunbookStep(2, "Scale APIM units", "Add capacity units to APIM instance", automated=True,
                        command=f"az apim update --name {settings.apim_service_name} --resource-group {settings.azure_resource_group} --sku-capacity 2"),
    ...
    IncidentPlan(
        ...
        runbook=[
            RunbookStep(1, "Check model deployment", "Verify model endpoint is healthy", automated=True,
                        command=f"az cognitiveservices account show --name {settings.ai_foundry_name} --resource-group {settings.azure_resource_group} --query '{{provisioningState:properties.provisioningState}}'"),
    ...
    IncidentPlan(
        ...
        runbook=[
            ...
            RunbookStep(3, "Restart frontend", "Restart the App Service", automated=True,
                        command=f"az webapp restart --name {settings.frontend_app_service_name} --resource-group {settings.azure_resource_group}"),
